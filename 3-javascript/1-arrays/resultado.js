...
function transmitiendoBatch() {
    ...
    let batchSize = Math.ceil(backup.length / 4); // Usamos Math.ceil para asegurar que el último lote no sea más pequeño que los otros
    let batchPosition = 0;

    for (let i = 1; i <= 4; i++) {
        const batchBackup = backup.slice(batchPosition, batchSize * i);
        batchPosition = batchSize * i; // Ajustamos la posición del próximo lote
